...
public class Lantern : CollectiblesManager
{
	...
	private void Update()
	{
		...
		setLightIntensity();
		if (!GameSettings.darkness && !isLighting)
		{
			if (isActive)
			{
				activateDeactivateLights(true, gameObject);
			}
		}
		else
		{
			activateDeactivateLights(false, gameObject);
		}
	}
	...
	{
		if (GamePropieties.lights < 3)
		{
            GameSettings.PlaySound(collectedSound);
			MazeSaver.SaveLightCondition(0);
	...
	public async void LightMaze(bool button)
	{
        ...
        if (GamePropieties.lights > 0 && !isLighting && PauseMenu.isPauseMenu && !Finish.panels[0].activeSelf && !Finish.panels[1].activeSelf)
		{
			StartCoroutine(IlightMaze());
			GamePropieties.SetLights(GamePropieties.lights - 1);
		}
	}
	...
	private void setLightIntensity()
	{
		...
		{
			playerLights[i].pointLightOuterRadius = 0.75f + 0.2f * (float)GamePropieties.lightIntensity;
		}
	}
}
